...
public class Shot {
	...
	public Shot(double x, double y, double angle, double shipXVel, double shipYVel, int lifeLeft){
		this.x = x;
		this.y = y;
		// add the velocity of the ship to the shot velocity (so the shot's velocity is relative to the ship's velocity)
	...
	public void move(int scrnWidth, int scrnHeight){
		...
		if(x<0)				// wrap the shot around to the opposite side of the screen if needed
			x += scrnWidth;
		else if(x>scrnWidth)
			x -= scrnWidth;
		if(y<0)
	...
	public void draw(Graphics g){
		...
		g.fillOval((int)(x-.5),(int)(y-.5), 3, 3); //draw circle of radius 3 centered at the closest point 
